...
class ProductsScraper:
    ...
    def open_website(self, url: str):
        """
        Open the website specified by the given URL.
    ...
    def decline_cookies(self):
        ...
        :raises NoSuchElementException: Decline cookies button was not found on the webpage
        :raises ElementNotInteractableException: Raised when a cookies element is not iterable
        :raises TimeoutException: Raised when a command does not complete in enough time.
        :raises Exception: Raised when a command fail with unknown reasons
        """
        try:
            # Wait for the decline cookies button to be clickable and click it
            cookies_decline = WebDriverWait(self.driver, 10).until(
                exp.element_to_be_clickable((By.XPATH, '//*[@id="footer_tc_privacy_button_2"]'))
            )
            # decline cookies
    ...
    def _scroll_to_bottom(self, scroll_pause_time: float = 0.5, max_scroll_time: float = 60.0) -> None:
        """
    ...
    def get_raiting_filter(self, raiting: int = 5, sort: str ="Best%20Sellers"):
        """
        Query parameters -> products with:

        :param raiting: average rating 5 (by default) 
        :param sort: sorted by Bestsellers  (by default)
        """
        return f"/?prefn1=bvAverageRating&prefv1={raiting}&srule={sort}"


    def get_product_tiles(self) -> list:
        """
        Scrolls through the webpage and retrieves a list of URLs, each corresponding 
        to a different product tile on the page.
    ...
    def get_product_ingredients(self):
        ...
        Returns:
            list: A list of ingredients if found, otherwise an empty list.
    ...
    def get_product_data(self, product_links):
        """
        Iterate through all products links and return dictonary with:
            - brand
            - price
            - ingredients
